...
ServerConfig::ServerConfig(const ServerConfig & ref)
  :	_serverName(ref._serverName), _listenPort(ref._listenPort), \
	_locations(ref._locations), _clientMaxBodySize(ref._clientMaxBodySize), \
...
std::ostream & operator<<(std::ostream & o, const ServerConfig &ref)
{
    ...
    o   << std::endl
        ...
        << "_locations:" << std::endl;
    for (std::vector<LocationConfig>::const_iterator it = ref._locations.begin();
        it != ref._locations.end(); it++)
        o << *it << std::endl;
    return o;
}
